...
description: Anti-slop frontend skill for landing pages, portfolios, and redesigns. Configured for PassShare's Laravel 13 + Livewire 4 + Flux UI 2 + Tailwind v4 + Alpine stack. The agent reads the brief, infers the right design direction, and ships interfaces that do not look templated. Real design systems when applicable, audit-first on redesigns, strict pre-flight check.
---
...
Example reads:
- *"Reading this as: B2B SaaS landing for technical buyers, with a Linear-style minimalist language, leaning toward Blade + Tailwind v4 + Instrument Sans + restrained CSS motion."*
...
| **Laravel Livewire app (this project)** | **Flux UI 2 + Tailwind v4** | Official Livewire component library for app UI; marketing pages use Blade + Tailwind |
| Tailwind-based modern SaaS / AI marketing | Tailwind v4 utilities + `dark:` variant | Default for indie + small team builds |

**Honesty rule:** if the brief reads as one of the systems above, install and use the **official** package. Do not recreate its CSS by hand. Do not import a system's tokens but then override 90% of them.

**One system per project.** Do not mix Fluent React with Carbon in the same tree. Do not import shadcn/ui components into a Material 3 app. **In this project:** Flux for app surfaces, Blade + Tailwind for marketing/landing — do not hand-roll Flux-equivalent components in app UI.

### 2.B When the brief is an aesthetic, not a system
For these directions, there is **no single official package**. Build with Blade + Tailwind + Flux (app UI) or native CSS + Tailwind (marketing). Be honest in code comments about what is borrowed inspiration vs. official material.
...
Unless the design read picks a real design system (Section 2.A), these are the defaults for **this project** (PassShare):

### 3.A Stack
* **Backend:** Laravel 13 on PHP 8.5. Auth via Laravel Fortify (`resources/views/pages/auth/`, `resources/views/layouts/auth/`).
* **Views:** Blade in `resources/views/`. Three surfaces:
  * **Marketing / landing** — static Blade (see `welcome.blade.php`). CSS-only interactivity preferred; may load `@vite(['resources/css/app.css'])` without JS.
  * **App UI** — Livewire 4 view-based (MFC) components in `resources/views/pages/` (e.g. `⚡profile.blade.php`) inside Flux layouts (`resources/views/layouts/app/sidebar.blade.php`). Use `wire:navigate` for in-app navigation.
  * **Shared Blade** — anonymous/class components in `resources/views/components/` (e.g. `<x-app-logo>`).
* **UI components:** Flux UI 2 (free edition) via `<flux:*>` for app surfaces (dashboard, auth, settings). Do not use Flux Pro components. Marketing pages: Blade + Tailwind utilities + `--landing-*` tokens and utility classes in `resources/css/app.css` (`.landing-gradient`, `.landing-fade-in`, `.landing-cta-primary`, etc.).
* **Reactivity:** Livewire 4 for server-driven UI (`wire:model`, `wire:click`, `wire:loading`). Alpine.js (bundled with Livewire 4) for local UI state — register components in `resources/js/alpineComponents.js`. **Do NOT** import Alpine separately.
* **Styling:** Tailwind CSS v4 via `@tailwindcss/vite` in `vite.config.js`. **No `tailwind.config.js`** — tokens live in CSS. Entry: `resources/css/app.css` with `@import 'tailwindcss'`, `@import '../../vendor/livewire/flux/dist/flux.css'`, `@source` directives for Blade views, and `@theme` tokens. Dark mode: `@custom-variant dark (&:where(.dark, .dark *))`. App UI: `@fluxAppearance` in `resources/views/partials/head.blade.php` (with CSP nonce). Landing pages may lock a single theme via CSS variables instead of toggling.
* **Assets:** Vite 8 via `laravel-vite-plugin`. Vite inputs: `resources/css/app.css`, `resources/js/app.js`, `resources/js/passkeys.js`. App layouts include `@include('partials.head')` and `@include('partials.flux-scripts')` (`@fluxScripts`). Run `composer run dev` (server + queue + Vite) or `npm run dev` / `npm run build` after frontend changes.
* **Animation (priority order):**
  1. **CSS** — `@keyframes` + utility classes (see `.landing-fade-in` in `app.css`) for MOTION_INTENSITY 4-7.
  2. **Alpine** — `x-transition`, `x-show`, `x-cloak` for UI state changes.
  3. **CSS scroll-driven** — `animation-timeline: view()` for scroll reveals without JS.
  4. **GSAP + ScrollTrigger** — only when `MOTION_INTENSITY ≥ 8` AND explicitly added to `package.json`. Isolate in `resources/js/` modules with teardown cleanup. Never assume GSAP is installed.
* **Fonts:** `bunny()` in `vite.config.js` (Instrument Sans, weights 400/500/600). Output via `@fonts` in Blade head. Never add raw Google Fonts `<link>` tags.
* **Blade optimization:** Livewire Blaze (`livewire/blaze`) is installed — Flux components are optimized automatically. Follow existing anonymous component patterns for custom Blade components; activate `blaze-optimize` skill when tuning Blaze.

### 3.B State
* **Server state:** Livewire component properties and actions. Validate and authorize in actions as you would HTTP requests.
* **Local UI state:** Alpine `x-data` inline, or registered `Alpine.data()` in `resources/js/alpineComponents.js`.
* **Marketing pages:** Prefer `<details>`/CSS-only patterns before Alpine when interactivity is trivial (see mobile menu in `welcome.blade.php`).
* **Never** bind Livewire/Alpine reactive state to scroll position, mouse coordinates, or animation frames. Use CSS scroll-driven animations or a dedicated vanilla JS module with `requestAnimationFrame` outside reactive state.

### 3.C Icons
* **App UI:** Flux `<flux:icon>` / `<flux:button icon="...">` with **Heroicons** (Flux default). Search [heroicons.com](https://heroicons.com/) for exact names — do not guess.
* **Custom icons:** `php artisan flux:icon crown github` for Lucide icons not in Heroicons. Stored in `resources/views/flux/icon/`.
* **Marketing pages:** Use `<flux:icon>` when Flux scripts are loaded; otherwise use Heroicons SVG from the official set. Do NOT hand-roll icon paths.
* **One family per surface.** Heroicons in Flux surfaces; Lucide only for icons imported via `flux:icon`.
* **Standardize `stroke-width` globally** (e.g. `1.5` or `2.0`).
...
Before importing ANY 3rd-party library, check `package.json` and `composer.json`. If the package is missing, output the install command first. **Never** assume a library exists. Flux UI, Livewire, Blaze, and Fortify are Composer packages — verify with `composer.json`, not npm.

### 3.G Related Skills (this project)
Activate the domain skill alongside this one — do not improvise patterns they already cover:
* **`fluxui-development`** — app UI `<flux:*>` components, forms, modals, toasts.
...
* **`blaze-optimize`** — Blaze component optimization (already installed).
...
* **Sans font choice:**
  * **This project:** Instrument Sans (configured in `vite.config.js` via `bunny()` and `@theme` in `app.css`).
  * **Discouraged as default (other projects):** `Inter`. Pick `Geist`, `Outfit`, `Cabinet Grotesk`, `Satoshi`, or a brand-appropriate sans first.
...
* **Loading:** Skeletal loaders matching the final layout's shape (`<flux:skeleton>` in app UI, CSS skeleton utilities on marketing pages). Use `wire:loading` for Livewire actions. Avoid generic circular spinners.
...
* When using Flux appearance/theming, set dark mode ONCE via `@fluxAppearance` in `resources/views/partials/head.blade.php`. Do not let individual sections override the global theme. Marketing pages (e.g. `welcome.blade.php`) may use a locked single-theme palette via `--landing-*` CSS variables instead.
...
* **Magnetic Micro-physics:** Use when `MOTION_INTENSITY > 5` AND the brief reads premium / playful / agency. Implement with vanilla JS + `requestAnimationFrame` or CSS transforms — never bind to Livewire/Alpine reactive state. See Section 3.B.
* **Perpetual Micro-Interactions** (Pulse, Typewriter, Float, Shimmer, Carousel): Use when `MOTION_INTENSITY > 5` AND the section actively benefits from motion (status indicators, live feeds, AI-feel). **Not every card needs an infinite loop.** If a section is informational, leave it still. Prefer CSS `@keyframes` with `cubic-bezier(0.16, 1, 0.3, 1)` easing.
...
Requires GSAP in `package.json`. Isolate in `resources/js/` and initialize from Alpine `x-init` or a dedicated import in `app.js`.

```js
// resources/js/stickyStack.js — import gsap + ScrollTrigger only after npm install
...
```blade
{{-- Blade markup --}}
<div x-data x-init="initStickyStack($el)" class="relative">
    @foreach ($cards as $card)
        <div class="stack-card sticky top-0 flex min-h-[100dvh] items-center justify-center">
            {{ $card }}
        </div>
    @endforeach
</div>
...
Requires GSAP in `package.json`. Same isolation rules as 5.A.

```js
// resources/js/horizontalPan.js
import { gsap } from 'gsap';
import { ScrollTrigger } from 'gsap/ScrollTrigger';
...
```blade
<section x-data x-init="initHorizontalPan($el, $refs.track)" class="relative overflow-hidden">
    <div x-ref="track" class="flex h-[100dvh] items-center">
        {{ $slot }}
    </div>
</section>
```
...
For simple "items appear as they enter viewport" (no pinning), prefer CSS over GSAP — lighter, no ScrollTrigger needed. This project already uses this pattern in `resources/css/app.css`:
...
.landing-fade-in { animation: landing-fade-in 0.7s ease-out both; }
.landing-fade-in-delay-1 { animation-delay: 0.15s; }
.landing-fade-in-delay-2 { animation-delay: 0.3s; }
```
...
<ul class="grid gap-6">
    @foreach ($items as $index => $item)
        <li @class([
            'landing-fade-in',
            'landing-fade-in-delay-'.min($index + 1, 4) => $index > 0,
        ])>
            {{ $item }}
        </li>
    @endforeach
...
For scroll-triggered reveals (not load-in), use CSS `animation-timeline: view()` or IntersectionObserver in a vanilla JS module. Save GSAP for actual pin/scrub work.

### 5.D Forbidden Animation Patterns

* **`window.addEventListener("scroll", ...)`** is banned. It runs on every scroll frame, jank-prone, no batching. Use GSAP ScrollTrigger, IntersectionObserver, CSS `scroll-driven animations` (`animation-timeline: view()`), or Alpine `@scroll.window` only for discrete toggles (not per-frame animation).
* **Custom scroll progress calculations bound to Livewire/Alpine reactive state.** Same reason — triggers re-renders on every frame.
* **`requestAnimationFrame` loops that mutate Livewire/Alpine state.** Keep animation in CSS or vanilla JS modules outside reactive state.
* **Layout transitions:** Use Alpine `x-transition` or CSS transitions for visible state changes (modals, dropdowns, expanding panels). Do not animate layout properties (`width`, `height`, `top`, `left`).
* **Staggered orchestration:** Use CSS cascade (`animation-delay: calc(var(--index) * 100ms)`) or sequential utility classes (`.landing-fade-in-delay-1`, etc.) for reveal moments where sequence matters.
...
* In JS modules: check `window.matchMedia('(prefers-reduced-motion: reduce)').matches` before initializing GSAP or scroll hijacks.
...
* Use Tailwind `dark:` variant (via `@custom-variant dark` in `app.css`) OR CSS variables for tokens. Pick one strategy per project. App UI: `@fluxAppearance` handles Flux theming.
...
* **LCP** < 2.5s. Hero image must use `loading="eager"` + `fetchpriority="high"`, or be preloaded via `<link rel="preload">`. Vite-processed assets via `@vite` or `Vite::asset()`.
* **INP** < 200ms. Heavy work off main thread (Web Workers for crypto — see `resources/js/cryptography/`).
...
* Be aware of bundle size. GSAP and Three.js are large — only add when justified. Lazy-load anything that's not above-the-fold via dynamic `import()` in `resources/js/`.
...
* **8-10 (Advanced Choreography):** Complex scroll-triggered reveals, parallax, scroll-driven animation (CSS `animation-timeline` or GSAP ScrollTrigger). Isolate GSAP in `resources/js/` modules. **NEVER use `window.addEventListener('scroll')`** - it is a hard ban, not a "prefer-not." See Section 5.D for the allowed alternatives.
...
* **Tailwind `dark:` variant** (default for this project): every color utility paired with its dark variant (`bg-white dark:bg-zinc-950`, `text-gray-900 dark:text-gray-100`). Configured via `@custom-variant dark` in `resources/css/app.css`.
* **CSS variables** (for landing pages and semantic tokens): define in `@theme` or `@layer base` (see `--landing-*` tokens in `app.css`) and swap values under `.dark` or `@media (prefers-color-scheme: dark)`.
* **Flux appearance:** App UI uses `@fluxAppearance` — do not fight Flux tokens with conflicting inline styles.
...
* **NO hand-rolled SVG icons.** Use Flux `<flux:icon>`, Heroicons, or Lucide via `php artisan flux:icon`. See Section 3.C.
...
* **Flux customization:** Allowed, but NEVER in default state. Customize radii, colors, shadows, typography to the project aesthetic. Use `<flux:button variant="primary">` etc. — do not recreate Flux components by hand.
...
* **CSS `@keyframes` + Tailwind utilities** — default for landing pages and MOTION_INTENSITY 4-7 (see `.landing-fade-in` in `app.css`).
* **Alpine `x-transition`** — for UI state changes (modals, dropdowns, toggles).
* **CSS scroll-driven animations** — `animation-timeline: view()` for scroll reveals without JS.
* **GSAP + ScrollTrigger** — for full-page scrolltelling and scroll hijacks at MOTION_INTENSITY 8-10. Isolate in `resources/js/` with teardown cleanup. Must be in `package.json` first.
* **Three.js / WebGL** — for canvas backgrounds and 3D scenes. Same isolation rule. Must be in `package.json` first.
* **NEVER mix GSAP scroll hijacks with Alpine reactive scroll listeners** in the same section — they fight over the same frames.
...
### 12.A File Location
```
.agents/skills/design-taste-frontend/blocks/
  hero/
    asymmetric-split.md
...
stack: ["laravel", "livewire", "flux", "blade", "tailwind", "alpine"]
---
```

### 12.C Required Body Sections
1. **Visual sketch** - short ASCII or description of the layout.
2. **Props API** - the Blade partial / Livewire component interface (`@props`, public properties).
3. **Code sketch** - minimal working Blade implementation (static Blade default, Alpine island or Livewire component for interactivity).
...
* Dashboards / dense product UI / admin panels — in **this project**, use Flux UI (activate `fluxui-development` skill). For non-Laravel briefs, see Fluent, Carbon, Atlassian, or Polaris in Section 2.A.
* Data tables (use `<flux:table>` in this project, or TanStack Table / AG Grid elsewhere).
...
- [ ] **No `window.addEventListener('scroll')`** - using ScrollTrigger / IntersectionObserver / CSS scroll-driven animations only?
- [ ] **Reduced motion** wrapped for everything `MOTION_INTENSITY > 3`?
- [ ] **Dark mode** tokens defined and tested in both modes?
- [ ] **Mobile collapse** explicit (`w-full`, `px-4`, `max-w-7xl mx-auto`) for high-variance layouts?
- [ ] **Viewport stability**: `min-h-[100dvh]`, never `h-screen`?
- [ ] **GSAP modules** have teardown cleanup (`ctx.revert()`) on Alpine destroy or page leave?
- [ ] **Empty / loading / error** states provided (`wire:loading`, `<flux:skeleton>`, Alpine `x-show`)?
- [ ] **Cards omitted** in favor of spacing where possible?
- [ ] **Icons** from Flux/Heroicons/Lucide only (Section 3.C), no hand-rolled SVG paths?
- [ ] **Vite assets** included via `@vite` and built with `npm run build`?
- [ ] **No AI Tells** from Section 9 (Inter as default, AI-purple, three-equal cards, Jane Doe, Acme, "Quietly in use at")?
- [ ] **Core Web Vitals** plausibly hit (LCP < 2.5s, INP < 200ms, CLS < 0.1)?
- [ ] **One design system** per surface (Flux for app UI, no hand-rolled Flux equivalents)?
...
```bash
# PassShare stack (already installed via Composer — verify before re-installing)
composer require livewire/flux livewire/livewire livewire/blaze laravel/fortify

# Import custom Lucide icons into Flux
php artisan flux:icon crown github

# GSAP (only when MOTION_INTENSITY ≥ 8 and scroll hijacks are required)
npm install gsap
...
### Tailwind (this project — v4, CSS-first)
- https://tailwindcss.com/docs/dark-mode
- https://tailwindcss.com/blog/tailwindcss-v4
- Activate `tailwindcss-development` skill — no `tailwind.config.js`; tokens in `resources/css/app.css` `@theme`

### Flux UI (this project)
- https://fluxui.dev/
- Activate `fluxui-development` skill for component patterns
- Use `search-docs` with `packages: ["livewire/flux"]` before customizing components

### Livewire
- https://livewire.laravel.com/docs
- Activate `livewire-development` skill for reactive UI
- Alpine.js is bundled — do not install separately
